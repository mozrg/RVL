...
window.onload = function () {
    ...
    el("btn-update").onclick        = function () { setUpdatingState(true); sendCmd("CMD:update"); };
...
function showUpdateBar(version, changelog) {
    ...
    if (txt) {
        var msg = String.fromCharCode(8593) + " Update available: v" + version;
        if (changelog) msg += " - " + changelog;
        txt.textContent = msg;
    }
...
function hideUpdateBar() {
    var bar = el("update-bar");
    if (bar) bar.className = "update-bar";
}
...
function setUpdatingState(updating) {
    ...
    if (updating) {
        ...
        btn.textContent = String.fromCharCode(8593) + " UPDATE";
        btn.disabled = false;
        btn.style.opacity = "1";
